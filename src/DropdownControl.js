...
function DropdownControl(props) {
  ...
  function addSelect() {
    ...
    props.handleAddItems(value); // Here am handling/passing values from child to parent Component
    setOptionsForSelectAdd(optionsAdd);
    const optionObjectToPush = {
      country: value,
      "official-name": value,
      name: value,
      "citizen-names": value,
    };
    leftOptions.unshift(optionObjectToPush);
    setOptions(leftOptions);
  }
    ...
    <div className="smart-dropdown-container">
      <div className="dropdown1">
        ...
        >
          ...
          {!seeMore ? (
            <Option value="SEE_ALL">
              {options.length > 0 ? options.length - 4 : null} More...
            </Option>
          ) : null}
        </Select>
      </div>
